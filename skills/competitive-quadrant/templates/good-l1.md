...
competitive-quadrant report with the required Mermaid quadrant figure — but
opaque to a machine consumer. It cannot be queried for "is this placement
still current?", "where did each vendor score come from?", or "what does this
relate to?". Compare templates/good.md (full L3: temporal validity, W3C-PROV
provenance, per-claim citations, typed relationships).
